                        ...
                        <div class="panel-body">
                            @if(Session::has('success-message'))
                                ...
                                @if ($errors->any())
                                    ...
                                    @endforeach
                                @endif
                         <form id="form" action="{{ route('postCarFuelTypeEdit',['id' => $result->id,'code' => $module->code]) }}" class="form-horizontal" method="post" accept-charset="utf-8">
                            @csrf
                            <fieldset class="content-group">
                                <div class="form-group">
                                    <label class="control-label col-lg-2">Ad</label>
                                    <div class="col-lg-10">
                                        <input name="name" type="text" class="form-control" required value="{{$result->name}}" placeholder="Ad">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="control-label col-lg-2">Sıralama</label>
                                    <div class="col-lg-10">
                                        <input name="sort" type="number" class="form-control" required value="{{$result->sort}}" placeholder="Sıralama">
                                    </div>
                                </div>
                                <div class="form-group">
