...
@section('addLink')
    <a href="{{ route('getSettingPriorityStrategyEdit',['id' => 0,'code' => $module->code]) }}"
                ...
                <div class="col-md-12">
                    <div class="panel panel-white">
                        <div class="panel-heading">
                            <h5 class="panel-title">Prioritet Strategiyası</h5>
                        </div>
                        <div class="panel-body">
                            <form
                                action="{{ route('getModuleSearch',['code'=> $module->code,'viewMain' => 'setting','view'=>'setting-priority-strategy.setting-priority-strategy' ]) }}"
                                        ...
                                        <div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input name="from_time" type="text" class="form-control"
                                                       id="anytime-time" value="{{request()->input('from_time')}}">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input name="to_time" type="text" class="form-control"
                                                       id="anytime-time1" value="{{request()->input('to_time')}}">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <input name="priority" type="number" class="form-control"
                                                   value="{{request()->input('priority')}}" placeholder="Prioritet">
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                ...
                                <div class="table-responsive">
                                    ...
                                    <table class="table table-bordered table-striped table-hover"
                                        ...
                                        <tr>
                                            ...
                                            <th>
                                                <a href="{{ Url('module/search/'.$module->code.'/setting/setting-priority-strategy.setting-priority-strategy') }}?_token={{ csrf_token() }}&from_time={{ request()->get('from_time') }}&to_time={{ request()->get('to_time') }}&&action={{ request()->get('action') }}&&priority={{ request()->get('priority')}}&status={{ request()->get('status') }}&&column_name=from_time&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}"><i
                                                        class="@if( request()->get('column_name') == 'from_time') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                    Başlama Vaxtı</a></th>

                                            <th>
                                                <a href="{{ Url('module/search/'.$module->code.'/setting/setting-priority-strategy.setting-priority-strategy') }}?_token={{ csrf_token() }}&from_time={{ request()->get('from_time') }}&to_time={{ request()->get('to_time') }}&&action={{ request()->get('action') }}&&priority={{ request()->get('priority')}}&status={{ request()->get('status') }}&&column_name=to_time&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}"><i
                                                        class="@if( request()->get('column_name') == 'to_time') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                    Bitmə Vaxtı</a></th>

                                            <th>
                                                <a href="{{ Url('module/search/'.$module->code.'/setting/setting-priority-strategy.setting-priority-strategy') }}?_token={{ csrf_token() }}&from_time={{ request()->get('from_time') }}&to_time={{ request()->get('to_time') }}&&action={{ request()->get('action') }}&&priority={{ request()->get('priority')}}&status={{ request()->get('status') }}&&column_name=action&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}"><i
                                                        class="@if( request()->get('column_name') == 'action') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                    Əməliyyat</a></th>
                                            <th>
                                                <a href="{{ Url('module/search/'.$module->code.'/setting/setting-priority-strategy.setting-priority-strategy') }}?_token={{ csrf_token() }}&from_time={{ request()->get('from_time') }}&to_time={{ request()->get('to_time') }}&&action={{ request()->get('action') }}&&priority={{ request()->get('priority')}}&status={{ request()->get('status') }}&&column_name=priority&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}"><i
                                                        class="@if( request()->get('column_name') == 'priority') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                    Prioritet</a></th>

                                            <th>
                                                <a href="{{ Url('module/search/'.$module->code.'/setting/setting-priority-strategy.setting-priority-strategy') }}?_token={{ csrf_token() }}&from_time={{ request()->get('from_time') }}&to_time={{ request()->get('to_time') }}&&action={{ request()->get('action') }}&&priority={{ request()->get('priority')}}&status={{ request()->get('status') }}&&column_name=payment_method&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}"><i
                                                        class="@if( request()->get('column_name') == 'payment_method') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                    Ödəmə</a></th>

                                            <th>
                                                <a href="{{ Url('module/search/'.$module->code.'/setting/setting-priority-strategy.setting-priority-strategy') }}?_token={{ csrf_token() }}&from_time={{ request()->get('from_time') }}&to_time={{ request()->get('to_time') }}&&action={{ request()->get('action') }}&&priority={{ request()->get('priority')}}&status={{ request()->get('status') }}&&column_name=status&order_type={{ request()->get('order_type') == 'asc' ? 'desc' : 'asc' }}&perPage={{ request()->get('perPage') }}"><i
                                                        class="@if( request()->get('column_name') == 'status') sort-amount-up icon-sort-amount-desc @endif"></i>
                                                    Status</a></th>
                                        ...
                                        @foreach($results as $result)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($result->action == 1) Tamamlanıb @endif
                                                    @if($result->action == 2) Ləğv edilib @endif
                                                    @if($result->action == 3) Vaxtı bitib @endif
                                                    @if($result->action == 4) Dispetçer tərəfindən və ya taksi özünin
                                                    sifarişi ləğv etməsi @endif
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <a class="btn btn-default"
                                                           href="{{ route('getSettingPriorityStrategyEdit',['id' => $result->id,'code' => $module->code]) }}"><i
